...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            ...
            pixAcc, mIoU, f_one, dice, se, _ = self._get_seg_metrics().values()

            # PRINT INFO
            tbar.set_description('TRAIN ({}) | Loss: {:.3f} | Acc {:.4f} mIoU {:.4f} F_One {:.4f} Dice {:.4f} Se {:.4f} | B {:.2f} D {:.2f} |'.format(
                                                epoch, self.total_loss.average,
                                                pixAcc, mIoU, f_one, dice, se,
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target) in enumerate(tbar):
                ...
                # PRINT INFO
                pixAcc, mIoU, f_one, dice, se, _ = self._get_seg_metrics().values()
                tbar.set_description('EVAL ({}) | Loss: {:.3f}, PixelAcc: {:.4f}, Mean IoU: {:.4f} F_One {:.4f} Dice {:.4f} Se {:.4f} |'.format(epoch,
    ...
    def _reset_metrics(self):
        ...
        self.total_fp, self.total_fn = 0, 0

    def _update_seg_metrics(self, correct, labeled, inter, union, fp, fn):
    ...
    def _get_seg_metrics(self):
        ...
        f_one = 1.0 * self.total_inter / (np.spacing(1) + self.total_inter + 0.5 * (self.total_union - self.total_inter))
        f_one = f_one.mean()

        dice = 2.0 * (self.total_inter + 1) / (np.spacing(1) + self.total_inter + self.total_union + 1)
        dice = dice.mean()

        se = 1.0 * self.total_inter / (np.spacing(1) + self.total_inter + self.total_fn)
        se = se.mean()
        ...
        return {
            ...
            "F_One": np.round(f_one, 4),
            "Dice": np.round(dice, 4),
            "Sensitivity": np.round(se, 4),
