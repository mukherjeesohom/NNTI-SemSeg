...
def batch_intersection_union(predict, target, num_class, labeled):
    ...
    fp = area_pred - area_inter
    fn = area_lab - area_inter
    assert (area_inter <= area_union).all(), "Intersection area should be smaller than Union area"
    return area_inter.cpu().numpy(), area_union.cpu().numpy(), fp.cpu().numpy(), fn.cpu().numpy()
...
def eval_metrics(output, target, num_class):
    ...
    inter, union, fp, fn = batch_intersection_union(predict, target, num_class, labeled)
    return [np.round(correct, 5), np.round(num_labeled, 5), np.round(inter, 5), np.round(union, 5), np.round(fp, 5), np.round(fn, 5)]
